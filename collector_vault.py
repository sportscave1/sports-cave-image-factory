...
def _asset_signing_secret():
    ...
    if configured:
        return configured
    candidates = _session_token_secrets()
    if candidates:
        return candidates[0]
...
FRAME_PRODUCT_QUERY = """
query CollectorVaultFrameProduct($query: String!) {
...
def get_frame_product(*, force=False):
    ...
    cache_key = (handle, configured_variant)
    now_value = time.time()
    with _CACHE_LOCK:
    ...
    try:
        ...
        matches = [
            ...
            if str(product.get("handle") or "") == handle and str(product.get("status") or "") == "ACTIVE"
        ]
        product = matches[0] if len(matches) == 1 else {}
        variants = (product.get("variants") or {}).get("nodes") or []
        if configured_variant:
            variants = [variant for variant in variants if variant.get("id") == configured_variant]
        selected_variant = variants[0] if len(variants) == 1 else {}
        value = {
            "product_id": str(product.get("id") or ""),
            "handle": str(product.get("handle") or ""),
            "variant_id": str(selected_variant.get("id") or ""),
            "available": bool(product and selected_variant),
        }
    with _CACHE_LOCK:
...
def _judgeme_configured():
    return bool(str(os.getenv("JUDGEME_PRIVATE_API_TOKEN") or "").strip())
...
def _reserve_review_submission(owner_hash, row, rating, title, body):
    with supabase_backend.connect() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
            ...
            if existing:
                cur.execute(
                    ...
                    SET rating=%s, review_title=%s, review_body=%s,
                        status='submitting', last_error=NULL, updated_at=now()
                    WHERE id=%s
                    RETURNING *
                    """,
                    (rating, title or None, body, existing.get("id")),
                )
            else:
                cur.execute(
                    """
                    INSERT INTO collector_reviews (
                        ...
                        shopify_order_id, shopify_product_id, rating, review_title, review_body
                    )
...
def _submit_judgeme_review(row, submission, *, name, email, photo_upload, request_post=None):
    ...
    return {
        ...
        "verification_status": str(
            review.get("verified") or review.get("verification_status") or review.get("source") or ""
        ),
...
def submit_review(
    ...
    *,
    ...
    submission = _reserve_review_submission(owner_hash, row, rating_value, title_value, body_value)
    photo_upload = {}
    try:
