...
def audit(conn):
    ...
    with conn.cursor() as cur:
        ...
        orders = query(cur, "SELECT shopify_order_id, order_name, financial_status, cancelled_at, "
                       "processed_at, synced_at, to_jsonb(o)->>'ingestion_status' AS ingestion_status, "
                       "to_jsonb(o)->>'ingestion_result' AS ingestion_result, "
                       "(COALESCE(to_jsonb(o)->>'ingestion_reason', '') <> '') AS has_ingestion_error "
                       "FROM shopify_orders o WHERE shopify_order_id IN (%s, %s)", order_identity)
