...
for(const name of ['20260626_ads_intelligence_v1.sql','20260626_ads_intelligence_v2_breakdowns.sql','20260626_ads_product_mapping_v1.sql','20260912235112_meta_review_reporting.sql'])
...
const productCommands=JSON.parse(execFileSync('.venv/Scripts/python.exe',['-m','tests.meta_review_sql_fixture','--product-confirm'],{encoding:'utf8'}));
await db.exec('BEGIN');
for(const command of productCommands) await executeCommand(command);
await db.exec('COMMIT');
const mappingInsert=productCommands.find(c=>c.sql.startsWith('INSERT INTO ads_product_mapping'));
assert.equal((await executeCommand(mappingInsert)).rows.length,1);
const conflicting=[...mappingInsert.params]; conflicting[1]='another-product';
assert.equal((await executeCommand(mappingInsert,conflicting)).rows.length,0);
assert.equal((await db.query("SELECT product_handle FROM ads_product_mapping WHERE ad_id='1'")).rows[0].product_handle,'canonical-art');
const productDump=await db.dumpDataDir(); await db.close(); db=new PGlite({loadDataDir:productDump});
const enriched=(await executeCommand(handoffLookup)).rows[0].context;
assert.equal(enriched.product_mapping.product_handle,'canonical-art');
assert.equal(enriched.components.primary_text.value,'Exact primary\n\nNo rewriting');
assert.equal(enriched.image_sha256,'digest');
assert.equal(JSON.parse((await db.query("SELECT notes FROM ads_product_mapping WHERE ad_id='1'")).rows[0].notes).creative_id,'creative');
console.log('PASS actual product confirmation SQL, conflict guard and durable enriched handoff');
