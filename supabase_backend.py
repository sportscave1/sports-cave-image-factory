...
def canonical_shopify_id(value):
    raw = str(value or "").strip()
    if not raw:
        return ""
...
def _shopify_id_candidates(resource_type, value):
    ...
    candidates = {candidate for candidate in (raw, canonical) if candidate}
    if canonical:
        candidates.add(f"gid://shopify/{resource_type}/{canonical}")
...
def canonical_order_sync_key(shopify_order_id="", shopify_order_name=""):
    ...
    order_name = str(shopify_order_name or "").strip().upper()
    if order_name:
        return f"name:{order_name}"
    return ""
...
def _ensure_schema_uncached():
    ...
    with connect() as conn:
        with conn.cursor() as cur:
            ...
            if table_exists(cur, "edition_orders") and column_exists(cur, "edition_orders", "allocation_key"):
                cur.execute(
                    """
                ...
                cur.execute("CREATE INDEX IF NOT EXISTS idx_edition_orders_allocation_key ON edition_orders(allocation_key)")
                _safe_create_index(
                    cur,
                    """
                    ...
                    WHERE COALESCE(allocation_key, '') <> ''
                    """,
                    "idx_edition_orders_allocation_key_unique",
                )
            if table_exists(cur, "edition_order_sync_locks"):
                cur.execute(
                    """
                    ...
                    SELECT DISTINCT ON (canonical_order_key)
                           ...
                           shopify_order_name,
                           'allocated',
                           'Backfilled from existing edition_orders',
                           now()
                    FROM (
                        SELECT CASE
...
def _daily_task_terminal_outcome(task):
    task = dict(task or {})
    status = str(task.get("status") or "").strip().casefold()
    outcome = str(task.get("outcome") or "").strip().casefold()
    mapped = {
...
def _daily_task_has_content(task, task_type=""):
    task = dict(task or {})
    name = str(task.get("task") or task.get("title") or "").strip()
    if str(task_type or "").casefold() == "top":
        return bool(name)
    return bool(name or str(task.get("details") or "").strip())


def _finalise_overdue_daily_execution_sheets_locked(
    cur,
    user_id,
    local_today,
    *,
    ...
    """Finalise overdue plans while the caller owns the account transaction lock."""
    clean_user_id = str(user_id or "").strip()
    today = local_today if isinstance(local_today, date) else date.fromisoformat(str(local_today))
    now_utc = now_utc or datetime.now(timezone.utc)
    cur.execute(
        """
        SELECT *
        ...
        FOR UPDATE
        """,
        (clean_user_id, today.isoformat()),
    )
    sheet_rows = list(cur.fetchall() or [])
    sheet_ids = [str(row.get("id") or "") for row in sheet_rows if row.get("id")]
    timer_rows = []
    if sheet_ids:
        cur.execute(
            """
            ...
            WHERE user_id=%s AND sheet_id=ANY(%s::uuid[])
            ORDER BY updated_at DESC
            FOR UPDATE
            """,
            (clean_user_id, sheet_ids),
        )
        timer_rows = list(cur.fetchall() or [])

    timers_by_sheet = {}
    for row in timer_rows:
        timers_by_sheet.setdefault(str(row.get("sheet_id") or ""), []).append(row)

    finalised = []
    cleared_timer_ids = []
    for sheet_row in sheet_rows:
        sheet_id = str(sheet_row.get("id") or "")
        top_tasks = [dict(item or {}) for item in (sheet_row.get("top_tasks") or [])]
        additional_items = [dict(item or {}) for item in (sheet_row.get("additional_items") or [])]
        sheet_timers = timers_by_sheet.get(sheet_id, [])
        timer_lookup = {}
        for timer_row in sheet_timers:
            key = (str(timer_row.get("task_type") or ""), int(timer_row.get("task_index") or 0))
            timer_lookup.setdefault(key, timer_row)

        transitioned = []
        for task_type, tasks in (("top", top_tasks), ("additional", additional_items)):
            for task_index, task in enumerate(tasks):
                if not _daily_task_has_content(task, task_type):
                    continue
                terminal = _daily_task_terminal_outcome(task)
                timer_row = timer_lookup.get((task_type, task_index), {})
                ...
                transition = _daily_task_terminal_transition(
                    ...
                    timer,
                    DAILY_TIMER_OUTCOME_DID_NOT_FINISH,
                    reason="Automatically finalised after the work date ended.",
                    now_utc=now_utc,
                )
                tasks[task_index] = transition["task"]
                transitioned.append((task_type, task_index, transition, timer))
        ...
        for timer_row in sheet_timers:
            timer = _timer_from_row(timer_row)
            if not timer or timer.get("outcome") or str(timer.get("status") or "").casefold() not in DAILY_TIMER_ACTIVE_STATUSES:
                continue
            task = task_lookup.get((timer.get("task_type"), int(timer.get("task_index") or 0)), {})
            outcome = _daily_task_terminal_outcome(task) or DAILY_TIMER_OUTCOME_DID_NOT_FINISH
            elapsed = task.get("actual_elapsed_seconds")
            if elapsed is None:
                elapsed = _daily_timer_elapsed_seconds(timer, now_utc)
            ...
            cur.execute(
                ...
                SET status='completed',
                    ...
                    completion_method=COALESCE(NULLIF(completion_method, ''), %s),
                    skip_reason=COALESCE(skip_reason, %s),
                ...
                (
                    ...
                    remaining,
                    elapsed,
                    str(task.get("completion_method") or outcome),
                    str(task.get("skip_reason") or "") or None,
                    now_utc,
                    timer.get("id"),
                    clean_user_id,
                ),
            )
            cleared_timer_ids.append(str(timer.get("id") or ""))
        ...
        cur.execute(
            ...
            RETURNING *
            """,
            (json_dumps(top_tasks), json_dumps(additional_items), sheet_id, clean_user_id),
        )
        updated_sheet = cur.fetchone() or sheet_row
        for task_type, task_index, transition, timer in transitioned:
            task = transition["task"]
            event_entity = timer.get("id") or f"task:{sheet_id}:{task_type}:{task_index}"
            _insert_daily_timer_activity_once(
                cur,
                event_type="daily_planner_task_auto_finalised",
                timer_id=event_entity,
                sheet_id=sheet_id,
                message=f"Daily Planner task automatically marked did not finish: {task.get('task') or task.get('details') or 'Task'}",
                actor=actor,
                event_key=f"daily_planner_task_auto_finalised:{event_entity}:v{transition['outcome_version']}",
                metadata={
                    "task": task.get("task") or task.get("details") or "Daily Planner task",
                    "task_type": task_type,
                    "task_index": task_index,
                    "outcome": DAILY_TIMER_OUTCOME_DID_NOT_FINISH,
                    "audit_reason": "overdue_day_rollover",
                    "actor_id": clean_user_id,
                },
            )
        finalised.append(_daily_execution_sheet_from_row(updated_sheet))
    ...
    reminder = _daily_execution_sheet_from_row(cur.fetchone() or {})
    return {
        "finalised_sheets": finalised,
        "finalised_dates": [sheet.get("sheet_date") for sheet in finalised],
        "cleared_timer_ids": [timer_id for timer_id in cleared_timer_ids if timer_id],
        "review_reminder": reminder,
    }


def finalise_overdue_daily_execution_sheets(
    user_id,
    local_today,
    *,
    actor="sports_cave_os",
    ...
    with connect() as conn:
        with conn.cursor() as cur:
            cur.execute(f"SET LOCAL statement_timeout = {_dashboard_query_timeout_ms()}")
            cur.execute("SELECT pg_advisory_xact_lock(hashtextextended(%s, 0))", (clean_user_id,))
            result = _finalise_overdue_daily_execution_sheets_locked(
                cur,
                clean_user_id,
                local_today,
                actor=actor,
...
def start_daily_execution_task_timer(
    ...
    *,
    ...
    today = local_today if isinstance(local_today, date) else date.fromisoformat(
        str(local_today or datetime.now(ZoneInfo("Australia/Sydney")).date().isoformat())
    )
    with connect() as conn:
        with conn.cursor() as cur:
            cur.execute(f"SET LOCAL statement_timeout = {_dashboard_query_timeout_ms()}")
            cur.execute("SELECT pg_advisory_xact_lock(hashtextextended(%s, 0))", (clean_user_id,))
            rollover = _finalise_overdue_daily_execution_sheets_locked(
                cur,
                clean_user_id,
                today,
                actor=actor,
            ...
            if _daily_task_terminal_outcome(task):
                raise ValueError("This Daily Planner task is already closed.")
            sheet_date = sheet.get("sheet_date")
            if isinstance(sheet_date, str):
                sheet_date = date.fromisoformat(sheet_date)
            if sheet_date and sheet_date < today and not task.get("reopened_at"):
                raise ValueError("Reopen this historical task before starting its timer.")
            cur.execute(
                """
                SELECT *
                FROM daily_execution_task_timers
            ...
            active_rows = list(cur.fetchall() or [])
            same_active = {}
            replaced = []
            for active in active_rows:
                ...
                active_timer = _timer_from_row(active)
                if same_task and str(active_timer.get("status") or "").casefold() in {"running", "paused"}:
                    same_active = active
                    continue
                previous_sheet = _select_daily_sheet_for_timer(
                    cur,
                    clean_user_id,
                    active_timer.get("sheet_id"),
                    lock=True,
                )
                previous_task = _daily_sheet_task(
                    previous_sheet,
                    active_timer.get("task_type"),
                    active_timer.get("task_index"),
                )
                previous_outcome = _daily_task_terminal_outcome(previous_task)
                preserved_terminal = bool(previous_outcome)
                if not previous_outcome:
                    transition = _daily_task_terminal_transition(
                        previous_task,
                        active_timer,
                        DAILY_TIMER_OUTCOME_DID_NOT_FINISH,
                        reason="Replaced by the next Daily Planner task.",
                    )
                    previous_task = transition["task"]
                    previous_outcome = DAILY_TIMER_OUTCOME_DID_NOT_FINISH
                    previous_sheet_data = _daily_execution_sheet_from_row(previous_sheet)
                    column = "top_tasks" if active_timer.get("task_type") == "top" else "additional_items"
                    tasks = list(previous_sheet_data.get(column) or [])
                    previous_index = int(active_timer.get("task_index") or 0)
                    if previous_index < len(tasks):
                        tasks[previous_index] = previous_task
                        cur.execute(
                            f"""
                            UPDATE daily_execution_sheets
                            ...
                            WHERE id=%s AND user_id=%s
                            """,
                            (json_dumps(tasks), active_timer.get("sheet_id"), clean_user_id),
                        )
                    event_entity = active_timer.get("id") or f"task:{active_timer.get('sheet_id')}:{active_timer.get('task_type')}:{previous_index}"
                    _insert_daily_timer_activity_once(
                        cur,
                        event_type="daily_planner_task_replaced",
                        timer_id=event_entity,
                        sheet_id=active_timer.get("sheet_id"),
                        message=f"Daily Planner task replaced by the next task: {previous_task.get('task') or previous_task.get('details') or 'Task'}",
                        actor=actor,
                        event_key=f"daily_planner_task_replaced:{event_entity}:v{transition['outcome_version']}",
                        metadata={
                            "task": previous_task.get("task") or previous_task.get("details") or "Daily Planner task",
                            "outcome": previous_outcome,
                            "audit_reason": "replaced_by_next_task",
                            "actor_id": clean_user_id,
                        },
                    )
                elapsed = previous_task.get("actual_elapsed_seconds")
                if elapsed is None:
                    elapsed = _daily_timer_elapsed_seconds(active_timer)
                elapsed = max(int(elapsed or 0), 0)
                remaining = max(int(active_timer.get("allocated_seconds") or 0) - elapsed, 0)
                cur.execute(
                    """
                    UPDATE daily_execution_task_timers
                    SET status='completed',
                    ...
                    (
                        ...
                        remaining,
                        elapsed,
                        str(previous_task.get("completion_method") or previous_outcome),
                        str(previous_task.get("skip_reason") or "") or None,
                        active_timer.get("id"),
                        clean_user_id,
                    ),
                )
                stopped = cur.fetchone() or active
                replaced.append(
                    {
                        "timer": _timer_from_row(stopped),
                        "outcome": previous_outcome,
                        "preserved_terminal": preserved_terminal,
                        "audit_reason": "stale_terminal_lock_cleared" if preserved_terminal else "replaced_by_next_task",
                    }
                )
            if same_active:
                active_timer = _timer_from_row(same_active)
                active_timer.update(
                    {
                        "task": _daily_timer_task_name(sheet, clean_type, safe_index),
                        "sheet_date": str(sheet.get("sheet_date") or ""),
                        "remaining_seconds": _daily_timer_remaining_seconds(active_timer),
                        "elapsed_seconds": _daily_timer_elapsed_seconds(active_timer),
                    }
                )
                conn.commit()
                return {
            ...
            row = cur.fetchone()
            if not row:
                raise RuntimeError("Daily Planner could not acquire the timer lock for this task.")
    ...
    task.update(
        {
            ...
            "skip_reason": clean_reason if clean_outcome == DAILY_TIMER_OUTCOME_SKIPPED else "",
            "outcome_reason": clean_reason if clean_outcome == DAILY_TIMER_OUTCOME_DID_NOT_FINISH else "",
    ...
    with connect() as conn:
        with conn.cursor() as cur:
                ...
                history.append(
                    {
                        ...
                        "outcome_reason": task.get("outcome_reason") or "",
            ...
            cur.execute(
                ...
                SET {column}=%s::jsonb,
                    status=CASE
                        WHEN %s AND status='finalised' THEN 'reopened'
                        WHEN %s THEN 'active'
                        ELSE status
                    END,
                    updated_at=now()
...
def list_edition_products_read_only(search="", limit=500, offset=0):
    ...
    if search_value:
        operation = "edition_ops.products.search"
        search_sql = """
            WHERE LOWER(COALESCE(ep.product_title, '')) LIKE %s
               OR LOWER(COALESCE(ep.shopify_handle, '')) LIKE %s
               OR EXISTS (
    ...
    diagnostic = {
        **diagnostic,
        ...
        "row_count": len(rows),
        "limit": limit_value,
        "tables": ["edition_products", "edition_orders"],
    }
    _LAST_DATABASE_READ_DIAGNOSTIC.set(diagnostic)
    print(
        "PERF Edition Ops read-only products+edition_orders "
        f"{(time.perf_counter() - started):.3f}s rows={len(rows)} queries=1 "
        f"recovered={str(bool(diagnostic.get('recovered'))).lower()}",
        flush=True,
    )
    return rows
...
def _public_edition_metafield_values(metafields):
    values = {}
    for metafield in _public_edition_metafields(metafields):
        key = str(metafield.get("key") or "").strip()
        if key:
            values[key] = metafield.get("value")
    return values


def _shopify_bool_text(value):
    return "true" if bool(value) else "false"


def _shopify_product_mirror_values_from_payload(payload):
    edition_total = max(_int_value(payload.get("edition_total"), 100), 1)
    next_number = max(_int_value(payload.get("next_edition_number"), 1), 1)
    sold_count = min(max(_int_value(payload.get("sold_count"), max(next_number - 1, 0)), 0), edition_total)
    remaining_count = min(max(_int_value(payload.get("remaining_count"), max(edition_total - sold_count, 0)), 0), edition_total)
    is_sold_out = bool(payload.get("is_sold_out")) or remaining_count <= 0 or next_number > edition_total
    edition_status = str(payload.get("edition_status") or "").strip() or ("sold_out" if is_sold_out else "limited_release")
    edition_label = str(payload.get("edition_name") or payload.get("run_edition_name") or DEFAULT_EDITION_NAME).strip() or DEFAULT_EDITION_NAME
    edition_display_text = str(payload.get("edition_display_text") or "").strip()
    if not edition_display_text:
        edition_display_text = "Sold Out Archive" if is_sold_out else f"Next Available Edition {format_edition_display_number(next_number, edition_total)}"
    return {
        "edition_enabled": _shopify_bool_text(not bool(payload.get("is_archived"))),
        "edition_total": str(edition_total),
        "edition_next_number": str(next_number),
        "edition_sold_count": str(sold_count),
        "edition_remaining": str(remaining_count),
        "edition_status": edition_status,
        "edition_label": edition_label,
        "next_edition_number": str(next_number),
        "last_assigned_edition": str(max(_int_value(payload.get("last_assigned_edition"), 0), 0)),
        "sold_count": str(sold_count),
        "remaining_count": str(remaining_count),
        "is_sold_out": _shopify_bool_text(is_sold_out),
        "edition_display_text": edition_display_text,
    }


def _product_mirror_source_values(payload):
    mirror_values = _shopify_product_mirror_values_from_payload(payload)
    return {key: mirror_values.get(key) for key in CANONICAL_STOREFRONT_EDITION_METAFIELDS}


def _record_product_metafield_mirror_audit(
    shopify_handle,
    *,
    status,
    payload=None,
        ...
        with connect() as conn:
            with conn.cursor() as cur:
                _insert_audit_log(
                    ...
                    entity_type="edition_product",
                    entity_id=payload.get("id") or payload.get("edition_product_id") or "",
                    shopify_handle=shopify_handle,
                    old_value={
                        "shopify_metafields_before": _public_edition_metafield_values(before or []),
                        "last_mirror_synced_at": str(payload.get("metafields_synced_at") or ""),
                        "last_mirror_status": str(payload.get("metafields_sync_status") or ""),
                    },
                    new_value={
                        "status": status,
                        "supabase_source": _product_mirror_source_values(payload) if payload else {},
                        "shopify_metafields_after": _public_edition_metafield_values(after or []),
...
def preview_shopify_edition_metafield_mirror_for_handles(
    handles,
    config=None,
    request_post=None,
    *,
    ...
    for handle in handles or []:
        ...
        if clean_handle and clean_handle not in seen:
    ...
    for handle in unique_handles:
        try:
            ...
            before_values = _public_edition_metafield_values(before_snapshot.get("metafields") or [])
            new_values = _shopify_product_mirror_values_from_payload(payload)
            keys = list(CANONICAL_STOREFRONT_EDITION_METAFIELDS) + [
                key for key in LEGACY_STOREFRONT_EDITION_METAFIELDS if key in new_values
            ]
            changes = [
                {
                    "key": key,
                    "shopify_before": before_values.get(key, ""),
                    "supabase_after": new_values.get(key, ""),
                    "will_update": str(before_values.get(key, "")) != str(new_values.get(key, "")),
                }
                for key in keys
            ]
            previews.append(
                {
                    "handle": handle,
                    "shopify_product_id": owner_gid,
                    "product_title": payload.get("product_title") or payload.get("title") or handle,
                    "last_mirror_synced_at": payload.get("metafields_synced_at") or "",
                    "last_mirror_status": payload.get("metafields_sync_status") or "",
                    "source_values": _product_mirror_source_values(payload),
                    "changes": changes,
    ...
    return {
        ...
        "legacy_keys": list(LEGACY_STOREFRONT_EDITION_METAFIELDS),
    }


def preview_shopify_edition_metafield_mirror_for_active_products(
    config=None,
    request_post=None,
    *,
    search="",
    ...
    if ensure_schema_first:
        ensure_schema()
    handles = _active_mapped_edition_product_handles(
        search=search,
        limit=limit,
        ensure_schema_first=False,
    )
    return preview_shopify_edition_metafield_mirror_for_handles(
        handles,
        config=config,
        request_post=request_post,
        ensure_schema_first=False,
...
def _product_metafield_sync_diagnostic(result, status="updated", error_message=""):
    ...
    return {
        ...
        "source_values": _product_mirror_source_values(payload) if payload else {},
        "last_mirror_synced_at": payload.get("metafields_synced_at") or "",
    ...
    *,
    ...
    try:
        ...
        _record_product_metafield_mirror_audit(
            ...
            status="updated",
            payload=payload,
            before=before_metafields,
            after=after_metafields,
        )
        return {
            "shopify_handle": shopify_handle,
            "payload": payload,
    ...
    try:
        with connect() as conn:
            with conn.cursor() as cur:
                ...
                existing = {str(row.get("shopify_order_id") or "").strip() for row in cur.fetchall() if row.get("shopify_order_id")}
                return {
                    candidate
                    for existing_id in existing
                    for candidate in _shopify_id_candidates("Order", existing_id)
                    if candidate
                }
    except Exception as error:
        schema_message = orders_sync_schema_error_message(error)
        ...
        with connect() as conn:
            with conn.cursor() as cur:
                ...
                return {
                    candidate
                    for existing_id in existing
                    for candidate in _shopify_id_candidates("LineItem", existing_id)
                    if candidate
                }
    except Exception as error:
        schema_message = orders_sync_schema_error_message(error)
...
def list_existing_edition_order_identities(orders, *, ensure_schema_first=True):
    if ensure_schema_first:
        ensure_schema()
    order_id_values = sorted(
        {
            candidate
            for order in (orders or [])
            for candidate in _shopify_id_candidates("Order", (order or {}).get("shopify_order_id"))
            if candidate
        }
    )
    order_names = sorted(
        {
            str((order or {}).get("order_name") or (order or {}).get("name") or "").strip()
            for order in (orders or [])
            if str((order or {}).get("order_name") or (order or {}).get("name") or "").strip()
        }
    )
    if not order_id_values and not order_names:
        return {"order_ids": set(), "order_names": set()}
    try:
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    OR (
                        ...
                        AND shopify_order_name = ANY(%s)
                    )
                    """,
                    (order_id_values, order_names),
                )
                rows = cur.fetchall()
    except Exception as error:
        schema_message = orders_sync_schema_error_message(error)
    ...
    existing_names = {
        str(row.get("shopify_order_name") or "").strip()
        for row in rows
        if str(row.get("shopify_order_name") or "").strip()
    }
    return {"order_ids": existing_ids, "order_names": existing_names}


def list_existing_order_sync_locks(orders, *, ensure_schema_first=True):
    if ensure_schema_first:
        ensure_schema()
    else:
        ensure_order_sync_lock_schema()
    keys = sorted(
        {
            key
            for order in (orders or [])
            for key in (
                canonical_order_sync_key((order or {}).get("shopify_order_id"), ""),
                canonical_order_sync_key("", (order or {}).get("order_name") or (order or {}).get("name")),
            )
            if key
        }
    )
    ...
    try:
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    SELECT canonical_order_key
                    FROM edition_order_sync_locks
                    WHERE canonical_order_key = ANY(%s)
                    """,
                    (keys,),
                )
                rows = cur.fetchall()
        ...
        if schema_message:
            raise RuntimeError(schema_message) from error
        raise
    return {str(row.get("canonical_order_key") or "").strip() for row in rows if row.get("canonical_order_key")}


def _order_sync_lock_keys(shopify_order_id="", shopify_order_name=""):
    keys = []
    id_key = canonical_order_sync_key(shopify_order_id, "")
    name_key = canonical_order_sync_key("", shopify_order_name)
...
def _ensure_order_sync_lock_table_with_cursor(cur):
    cur.execute(
        """
        CREATE TABLE IF NOT EXISTS edition_order_sync_locks (
            id BIGSERIAL PRIMARY KEY,
...
def ensure_order_sync_lock_schema():
    if not is_configured():
        return
    with connect() as conn:
        with conn.cursor() as cur:
...
def _find_existing_order_allocation_for_update(cur, shopify_order_id="", shopify_order_name=""):
    order_keys = _order_sync_lock_keys(shopify_order_id, shopify_order_name)
    order_id_candidates = _shopify_id_candidates("Order", shopify_order_id)
    cur.execute(
        """
        SELECT 'lock' AS source, id::text AS id, canonical_order_key, shopify_order_id, shopify_order_name, status
        FROM edition_order_sync_locks
        WHERE canonical_order_key = ANY(%s)
    ...
    cur.execute(
        """
        SELECT 'edition_order' AS source, eo.id::text AS id, eo.shopify_order_id,
               COALESCE(NULLIF(eo.shopify_order_name, ''), NULLIF(o.order_name, '')) AS shopify_order_name,
...
def _acquire_order_sync_lock(cur, *, shopify_order_id="", shopify_order_name="", status="allocated", reason=""):
    primary_key = canonical_order_sync_key(shopify_order_id, shopify_order_name)
    if not primary_key:
        raise ValueError("Cannot acquire edition order sync lock without an order ID or order name.")
    cur.execute(
        """
        INSERT INTO edition_order_sync_locks(
        ...
        ON CONFLICT (canonical_order_key) DO NOTHING
        RETURNING id::text AS id, canonical_order_key
        """,
        (primary_key, shopify_order_id or "", shopify_order_name or "", status or "allocated", reason or ""),
    )
    inserted = cur.fetchone()
    if not inserted:
        return None
    name_key = canonical_order_sync_key("", shopify_order_name)
    if name_key and name_key != primary_key:
        cur.execute(
            """
            INSERT INTO edition_order_sync_locks(
                canonical_order_key, shopify_order_id, shopify_order_name, status, reason, updated_at
            ...
            ON CONFLICT (canonical_order_key) DO NOTHING
            """,
            (name_key, shopify_order_id or "", shopify_order_name or "", status or "allocated", reason or ""),
        )
    return inserted
...
def _shopify_order_dedupe_key(order):
    if not isinstance(order, dict):
        return ""
    for key in ("shopify_order_id", "id", "admin_graphql_api_id"):
        value = str(order.get(key) or "").strip()
        if value:
            return value
    order_name = str(order.get("order_name") or order.get("name") or "").strip()
...
def _dedupe_shopify_order_candidates(*order_batches):
    orders = []
    by_key = {}
    duplicates = 0
    for batch in order_batches:
        for order in batch or []:
            if not isinstance(order, dict):
                continue
            key = _shopify_order_dedupe_key(order)
            if not key:
                orders.append(order)
                continue
            if key in by_key:
                duplicates += 1
                _merge_shopify_order_candidate(by_key[key], order)
                continue
            by_key[key] = order
            orders.append(order)
            ...
            with conn.cursor() as cur:
                for line_number, row in enumerate(rows or [], start=2):
                    ...
                    if edition_number:
                        ...
                        synthetic_allocation_key = allocation_identity_key(synthetic_order_id, synthetic_line_id, 1)
                        if overwrite_existing_orders:
                            cur.execute(
                                ...
                                INSERT INTO edition_orders(
                                    ...
                                    edition_number, edition_total, allocation_index, allocation_key, assigned_at, certificate_status
                                )
                                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, 1, %s, now(), 'Certificate Missing')
                                ON CONFLICT (shopify_line_item_id, allocation_index) DO UPDATE SET
                                    product_title=EXCLUDED.product_title,
                                    variant_title=EXCLUDED.variant_title,
                                    customer_name=EXCLUDED.customer_name,
                                    edition_total=EXCLUDED.edition_total,
                                    allocation_key=COALESCE(NULLIF(edition_orders.allocation_key, ''), EXCLUDED.allocation_key)
                                """,
                                (
                                    synthetic_order_id,
                                    synthetic_line_id,
        ...
        try:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    ...
                    LEFT JOIN shopify_orders o ON o.shopify_order_id=eo.shopify_order_id
                    WHERE (
                        {allocation_key_filter}
                        (
                            eo.shopify_order_id = ANY(%s)
                            AND eo.shopify_line_item_id = ANY(%s)
                            AND COALESCE(eo.allocation_index, 1)=%s
                        )
                    )
                    ORDER BY eo.assigned_at ASC NULLS LAST, eo.created_at ASC NULLS LAST
                    LIMIT 1
                    """,
                    (*allocation_key_params, order_id_candidates, line_item_id_candidates, allocation_index),
                )
                existing = cur.fetchone()
                if existing:
                    if has_allocation_key and allocation_key and not existing.get("allocation_key"):
                        cur.execute(
                            """
                            UPDATE edition_orders
                            ...
                            """,
                            (allocation_key, str(existing.get("id"))),
                        )
                    ...
                    if customer_name or customer_email:
                        cur.execute(
                            ...
                            (
                                ...
                                str(customer_email or "").strip(),
                                str(customer_email or "").strip(),
                                existing.get("shopify_order_id") or shopify_order_id,
                                existing.get("shopify_line_item_id") or shopify_line_item_id,
                                allocation_index,
                            ),
                        )
                ...
                if exact_existing:
                    ...
                    _acquire_order_sync_lock(
                        cur,
                        shopify_order_id=exact_existing.get("shopify_order_id") or shopify_order_id,
                        shopify_order_name=exact_existing.get("shopify_order_name") or exact_existing.get("order_name") or shopify_order_name,
                        reason="Backfilled from exact existing edition order.",
                    )
                    conn.commit()
                ...
                allocation_key_insert_params = (allocation_key,) if has_allocation_key else ()
                cur.execute(
                    f"""
                    INSERT INTO edition_orders(
...
def process_paid_order(
    ...
    *,
    ...
    for line_index, line_item in line_items_for_allocation:
        ...
        if line_created + line_existing + line_order_skipped >= quantity:
...
def _latest_paid_order_needs_sync(
    order,
    existing_order_ids,
    existing_line_item_ids,
    existing_order_states,
    existing_edition_order_identities=None,
    existing_order_sync_locks=None,
):
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                order_duplicate_groups AS (
                    SELECT 'order_duplicate' AS duplicate_type,
                ...
                SELECT
                    ...
                    dg.first_created_at,
                    dg.last_created_at,
                    k.shopify_order_name,
                    k.shopify_order_id,
                    k.shopify_line_item_id,
                    k.shopify_product_id,
                    k.shopify_variant_id,
                    k.shopify_handle,
                    k.product_title,
                    k.variant_title,
                ...
                JOIN keyed k ON (
                    ...
                    OR
                    (dg.duplicate_type = 'order_duplicate' AND ('order:' || k.canonical_order_identity) = dg.group_key)
                )
                ...
                ORDER BY dg.last_created_at DESC NULLS LAST
                LIMIT %s
                """,
                (max(min(int(limit or 50), 500), 1),),
            )
...
def sync_latest_paid_orders_to_supabase(
    ...
    *,
    ...
    if ensure_schema_first:
        ...
        _orders_sync_log("schema_check_skipped", "completed", mode="latest_paid_sync")
    ...
    try:
        ...
        if duplicate_group_count or diagnostics_blocked:
            ...
            return {
                ...
                "lines_already_existing": 0,
                "supabase_rows_inserted": 0,
                "assignments_created": 0,
                "edition_allocations_created": 0,
                "existing_assignments_skipped": 0,
                "existing_allocations_preserved": 0,
                "existing_allocation_units_skipped": 0,
                "duplicate_risk_units_blocked": duplicate_row_count,
                "edition_counters_incremented": 0,
    ...
    try:
        ...
        existing_edition_order_identities = list_existing_edition_order_identities(
            processable_orders,
            ensure_schema_first=ensure_schema_first,
        )
        existing_order_sync_locks = list_existing_order_sync_locks(
            processable_orders,
            ensure_schema_first=ensure_schema_first,
        )
        _orders_sync_log(
            ...
            edition_order_names=len(existing_edition_order_identities.get("order_names") or []),
            order_locks=len(existing_order_sync_locks),
        )
        _sync_perf_log(
        ...
        imported_orders = sum(
            1 for order in processable_orders if str(order.get("shopify_order_id") or "").strip() not in existing_order_ids
        )
        ...
        candidate_orders = [
            order
            for order in processable_orders
            if _latest_paid_order_needs_sync(
                order,
                existing_order_ids,
                existing_line_item_ids,
                existing_order_states,
                existing_edition_order_identities,
                existing_order_sync_locks,
            )
        ]
        _orders_sync_log(
            "candidate_orders_count",
            "completed",
            fetched_orders=seen,
        ...
        if backfill_latest_paid:
            ...
            _orders_sync_log("cursor_update_completed", "skipped", reason="backfill")
        elif candidate_processing_errors:
            success_timestamp = ""
            cursor_update_reason = (
                f"Cursor left unchanged because {candidate_processing_errors} candidate processing error(s) occurred."
            )
            _orders_sync_log(
                "cursor_update_completed",
                "skipped",
                reason="candidate_processing_errors",
                errors=candidate_processing_errors,
            )
            ...
            _orders_sync_log("cursor_update_completed", "skipped", reason="no_orders")
        ...
        return {
            ...
            "existing_allocation_units_skipped": existing_skipped,
            "duplicate_risk_units_blocked": 0,
            "edition_counters_incremented": assignments,
            ...
            "cursor_orders_fetched": int(payload.get("cursor_orders_fetched") or 0),
            "latest_created_orders_fetched": int(payload.get("latest_created_orders_fetched") or 0),
            "duplicate_orders_removed": int(payload.get("duplicate_orders_removed") or 0),
